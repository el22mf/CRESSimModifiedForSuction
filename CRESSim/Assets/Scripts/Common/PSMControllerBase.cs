...
public abstract class PSMControllerBase : MonoBehaviour
{
    ...
    public virtual void DriveJoints(float[] jointPositions)
    {
        ...
        for (int i = 0; i < m_robot.NumJoints; i++)
        {
            ...
            {
                Debug.Log("Joint limits reached, joint " + i + " is " + jointPositions[i] + " but limits are [" + m_robot.JointLimits[i].lower + ", " + m_robot.JointLimits[i].upper + "]");
        ...
        Debug.Log("Driving joints " + jointPositions[0] + " " + jointPositions[1] + " " + jointPositions[2] + " " + jointPositions[3] + " " + jointPositions[4] + " " + jointPositions[5] + " " + jointPositions[6]);
    ...
    [SerializeField]
    public PhysxArticulationRobot m_robot;
    [SerializeField]
    ...
    protected Quaternion m_rotationTooltipToJoint = new Quaternion(0f, 0f, 0f, 1f);//Quaternion(-0.5f, 0.5f, -0.5f, 0.5f);
    protected float[] m_jointPositionSetpoint;

    public IntPtr[] m_linkPtrs;
}
